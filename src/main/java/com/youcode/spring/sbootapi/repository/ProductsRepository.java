...
public interface ProductsRepository extends JpaRepository<Product, Long> {
    ...
    @Query("select new com.youcode.spring.sbootapi.models.extensions.ProductExtension(p.id, p.name, p.slug, p.price, p.stock, p.publishOn) from Product p")
    ...
    @Query("select new com.youcode.spring.sbootapi.models.extensions.ProductExtension(p.id, p.price) from Product p where p.id=:id")
    Product findByIdForPrice(@Param("id") Long id);

    @Query("select new com.youcode.spring.sbootapi.models.extensions.ProductExtension(p.id,p.name, p.slug, p.price) from Product p where p.id=:id")
    Product findByIdWithElementalInfo(@Param("id") Long id);

    @Query("select new com.youcode.spring.sbootapi.models.extensions.ProductExtension(p.id, p.name, p.slug, p.price) from Product p where p.id in :ids")
    ...
    @Query("select new com.youcode.spring.sbootapi.models.extensions.ProductExtension(p.id, p.name, p.slug, p.price,p.stock, p.publishOn) from Product p INNER JOIN p.tags t WHERE t.name= :tag")
